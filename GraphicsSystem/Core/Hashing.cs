...
namespace GraphicsSystem.Hashing
{
    public class Hashing
    {
        ...
        {
            return ComputeHash(Encoding.ASCII.GetBytes(plainText)).ToUpperInvariant();
        }

        public static bool verifyHash(string plainText, string cipherText)
        {
            string hashedPlainText = generateHash(plainText);
            return hashedPlainText == cipherText;
        }
        ...
        public static string ComputeHash(byte[] value)
        {
            ...
            string toreturn = Hex(hash.GetHash());
